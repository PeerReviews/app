...
$type = $question->type;
$answers = $DB->get_records('pr_answer', array('question_id' => $qcm_id));

$evaluations = $DB->get_records('pr_evaluation', array('question_id' => $qcm_id));

// Définir l'URL de la page et les informations de la page
$PAGE->set_url('/mod/peerreview/phase3_valorise_qcm.php', array('id' => $id, 'qcm_id' => $qcm_id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('edit_question', 'mod_peerreview') . " " . $type . "</p>";
echo "</div>";
...
echo "<div class='mt-8'>";
    ...
    echo "<div class='grid grid-cols-1 lg:grid-cols-3 gap-4'>";
        ...
        echo "<label for='referentiel_1' class='block font-semibold text-gray-700 text-lg'>" . get_string('referentiel', 'mod_peerreview') . " :</label>";
        echo "<select id='referentiel_1' name='questions[1][referentiel]' class='w-full p-2 mt-2 border border-gray-300 rounded-lg'>";
        echo "<option value=''>" . get_string('select_referentiel', 'mod_peerreview') . "</option>";
        $referentiels = $DB->get_records('pr_referentiel', ['sessionid' => $session->id]);
        ...
        echo "<div class='rounded-3xl bg-lime-200 mb-2 p-4 shadow-md'>";
        echo "<label for='competency_1' class='block font-semibold text-gray-700 text-lg'>" . get_string('competency', 'mod_peerreview') . " :</label>";
        echo "<select id='competency_1' name='questions[1][competency]' class='w-full p-2 mt-2 border border-gray-300 rounded-lg' disabled>";
        echo "<option value=''>" . get_string('select_competency', 'mod_peerreview') . "</option>";
        $competencies = $DB->get_records('pr_competency', ['sessionid' => $session->id]);
        ...
        echo "<div class='rounded-3xl bg-lime-200 mb-2 p-4 shadow-md'>";
        echo "<label for='subcompetency_1' class='block font-semibold text-gray-700 text-lg'>" . get_string('subcompetency', 'mod_peerreview') . " :</label>";
        ...
        echo "<option value=''>" . get_string('select_subcompetency', 'mod_peerreview') . "</option>";
        $subcompetencies = $DB->get_records('pr_subcompetency', ['sessionid' => $session->id]);
        ...
        echo get_string('add', 'mod_peerreview');
        echo "</button>";
        echo "</div>";
    ...
    echo "<label class='block font-semibold text-gray-700 text-lg'>" . get_string('keywords', 'mod_peerreview') . " :</label>";
    ...
    echo "<label for='context_1' class='block font-semibold text-gray-700 text-lg'>" . get_string('context', 'mod_peerreview') . " :</label>";
    ...
    // Question
    echo "<div class='rounded-3xl bg-indigo-200 my-4 p-4 shadow-md'>";
    echo "<label for='question_1' class='block font-semibold text-gray-700 text-lg'>" . get_string('question', 'mod_peerreview') . " :</label>";
    ...
    foreach ($answers as $index => $answer) {
        ...
        if ($type !== "TCS") {
            ...
            echo "<label for='answer_1_{$index}' class='col-span-2 block font-semibold text-gray-700 text-lg'>" . get_string('answer', 'mod_peerreview') . " $counter :</label>";
        ...
        if ($type != "TCS") {
            ...
            // Explication
            echo "<div class='py-2 grid grid-cols-12 w-full'>";
            echo "<label for='explanation_1_{$index}' class='col-span-2 block font-semibold text-gray-700 text-lg'>" . get_string('explanation', 'mod_peerreview') . " $counter :</label>";
            echo "<div class='col-span-10 w-full'>";
        ...
        echo "<div class='py-2 grid grid-cols-12 w-full'>";
        echo "<label for='correct_answer_1_{$counter}' class='col-span-2 block font-semibold text-gray-700 text-lg'>" . get_string('correct_answer', 'mod_peerreview') . " ?</label>";
...
echo "<div class='mb-4 mt-4 flex justify-end space-x-2'>";
    echo "<button type='submit' name='save' class='inline-block px-4 py-2 font-semibold rounded-2xl bg-lime-200 hover:bg-lime-300 cursor-pointer text-lime-700 no-underline text-lg'>" .
        get_string('save', 'mod_peerreview') . "</button>";
echo "</div>";  
...
if (empty($evaluations)) {
    ...
    echo "<p class='font-semibold text-xl'>" . get_string('no_evaluation_found', 'mod_peerreview') . "</p>";
    echo "</div>";
} else {
    $index = 1;
    foreach ($evaluations as $evaluation) {
        ...
        echo "<p class='font-semibold text-2xl text-gray-700 flex items-center gap-2 mb-4'>" .
            get_string('student_evaluation', 'mod_peerreview') . " " . $index . "</p>";
