...
if (!empty($studentid)) {
    $revisions = $DB->get_records('pr_evaluation', array('userid' => $studentid, 'status' => 1));
}


$productions = $DB->get_record('pr_assignedqcm', ['user_id' => $studentid, 'sessionid' => $session->id], 'prod1_id, prod2_id, prod3_id');
$nbTotal_revision = 0;
...
echo "<div class='flex justify-between items-center gap-4 mt-4 text-xl text-gray-700 font-semibold text-center'>";
    echo "<div class='flex-1 text-center rounded-3xl shadow-md p-4 bg-gray-50'>";
        echo "<p>" . get_string('student', 'mod_peerreview') . " " . $studentid . "</p>";
...
if ($revisions) {
    ...
    foreach ($revisions as $revision) {
        echo "<div class='bg-gray-50 p-4 rounded-lg flex flex-col h-full'>";
            ...
            echo "<div class='flex items-center gap-4 mt-auto'>";
                echo "<p class='text-gray-700 font-medium'>" . get_string('attributed_note_revision', 'mod_peerreview') . " :</p>";
    ...
    echo "<p class='text-center text-lg text-gray-600'>" . get_string('revision_not_found', 'mod_peerreview') . "</p>";
}
