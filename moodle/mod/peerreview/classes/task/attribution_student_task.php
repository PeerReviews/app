...
class attribution_student_task extends scheduled_task {
    ...
    public function execute(bool $force = false) {
        ...
        if (!$session || empty($session->start_date_2)) {
            mtrace("Erreur : date de lancement introuvable.");
            die();
        }

        // Vérifie si la tâche a déjà été effectuée
        if (!$force && $session->attribution_student_completed == 1) {
        ...
        $start_timestamp = strtotime($session->start_date_2);
        ...
        $DB->delete_records('pr_assignedqcm');
        $DB->execute("ALTER TABLE {pr_assignedqcm} AUTO_INCREMENT = 1");

        $students = $DB->get_records('user', null, '', 'id');
        $student_ids = array_keys($students);
        $num_students = count($student_ids);

        $nbReviewers = $session->nbreviewers;
        ...
        foreach ($student_ids as $student_id) {
            ...
            $DB->insert_record('pr_assignedqcm', $record);
