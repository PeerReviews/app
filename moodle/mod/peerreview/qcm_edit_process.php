...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($questions as $q_id => $question) {
        if (!empty(trim($question['question']))) {
            ...
            $existing_question = $DB->get_record('pr_question', ['id' => $q_id, 'sessionid' => $session->id]);
            if (!$existing_question) {
                throw new moodle_exception('invalidquestion', 'mod_peerreview');
            }
            ...
            $file_records = $file_storage->get_area_files(
                $context->id, 'mod_peerreview', $filearea, $q_id, 'sortorder', false
            );
            ...
            foreach ($file_records as $file) {
                $file_url = moodle_url::make_pluginfile_url(
                    $context->id, 'mod_peerreview', $filearea, $q_id,
            ...
            $DB->update_record('pr_question', $question_record);
            ...
            if (!empty($question['answers'])) {
                foreach ($question['answers'] as $i => $answer) {
                    ...
                    $existing_answer = $DB->get_record('pr_answer', [
                        'question_id' => $q_id,
                    ...
                    if ($existing_answer) {
                        ...
                        foreach($fileareas as $filearea){
                            ...
                            $file_records = $file_storage->get_area_files(
                                $context->id, 'mod_peerreview', $filearea, $existing_answer->id, 'sortorder', false
                            );
                            ...
                            foreach ($file_records as $file) {
                                $file_url = moodle_url::make_pluginfile_url(
                                    $context->id, 'mod_peerreview', $filearea, $existing_answer->id,
                                    $file->get_filepath(), $file->get_filename()
                                )->out();
                        ...
                        $DB->update_record('pr_answer', $answer_record);
                    } else {
                        // Insertion d'une nouvelle réponse
                        $DB->insert_record('pr_answer', $answer_record);
                    }
                }
            }
            ...
            if (!empty($question['keywords']) && is_array($question['keywords'])) {
                ...
                $existing_keywords = $DB->get_records('pr_question_keywords', ['question_id' => $q_id]);
                ...
                foreach ($keywords_to_add as $keyword_id) {
                    if (!empty(trim($keyword_id))) {
                        $existing_keyword = $DB->get_record('pr_keyword', ['id' => $keyword_id, 'sessionid' => $session->id]);
                        if ($existing_keyword) {
                            $relation_record = new stdClass();
                            $relation_record->question_id = $q_id;
                            $relation_record->keyword_id = $keyword_id;
                            $DB->insert_record('pr_question_keywords', $relation_record);
                        }
                    }
                }
                ...
                foreach ($keywords_to_remove as $keyword_id) {
                    ...
                    $DB->delete_records('pr_question_keywords', ['question_id' => $q_id, 'keyword_id' => $keyword_id]);
                }
            }
    ...
    if ($is_improved == 1) {
        redirect(new moodle_url('/mod/peerreview/phase3_qcm_list.php', array('id' => $id)), get_string('qcm_updated', 'mod_peerreview'), 10);
    } else {
        redirect(new moodle_url('/mod/peerreview/qcm_list.php', array('id' => $id)), get_string('qcm_updated', 'mod_peerreview'), 10);
    }
