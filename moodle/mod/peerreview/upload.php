...
try{ 
    ...
    if ($stored_file) {
        ...
        try {
        $file_record->id = $DB->insert_record('pr_file', $file_record);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Stored file in pr_file', 'details' => $e->getMessage()]);
            exit;
        }
        // Génération de l'URL du fichier
        $file_url = moodle_url::make_pluginfile_url(
            $context->id, 'mod_peerreview', $filearea, $itemid, '/', $filename
        )->out(false);
