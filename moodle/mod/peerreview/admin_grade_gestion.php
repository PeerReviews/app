...
$nbTotal_question = $session->nbqcm + $session->nbqcu + $session->nbtcs + $nbTotalQuestionPop;

require_login($course, true, $cm);

$PAGE->set_url('/mod/peerreview/admin_grade_gestion.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('student_list', 'mod_peerreview') . "</p>";
echo "</div>";
...
$columns = [
    ...
    'total_grade_questions' => get_string('total_grade_questions', 'mod_peerreview'),
    'total_grade_revisions' => get_string('total_grade_revisions', 'mod_peerreview'),
    'total_general' => get_string('total_general', 'mod_peerreview'),
...
$start_timestamp = $session->date_jury;
...
foreach ($students as $student) {
    ...
    $productions = $DB->get_record('pr_assignedqcm', ['user_id' => $student->userid], 'prod1_id, prod2_id, prod3_id');
    $nbTotal_revision = 0;

    if ($productions) {
        foreach ((array) $productions as $production_id) {
            if (!empty($production_id)) {
                $to_evaluate = $DB->get_records('pr_question', array('userid' => $production_id, 'status' => 1));
    ...
    echo '<tr id="row-' . $student->userid . '" class="border-t hover:bg-gray-50">';
        ...
        echo '<td class="px-3 py-4 text-md text-gray-600 flex space-x-2">';
            echo '<a href="show_production.php?id=' . $id . '&prod_id=' . $student->userid . '" 
                    ...
                    title="' . get_string('show_production', 'mod_peerreview') . '">
                    <i class="fas fa-p"></i>
                </a>';
