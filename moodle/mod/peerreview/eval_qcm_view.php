...
$question = $DB->get_record('pr_question', array('id' => $qcm_id, 'sessionid' => $session->id), '*', MUST_EXIST);
$referentiel = $DB->get_record('pr_referentiel', array('id' => $question->referentiel, 'sessionid' => $session->id), '*');
$competency = $DB->get_record('pr_competency', array('id' => $question->competency, 'sessionid' => $session->id), '*');
$subcompetency = $DB->get_record('pr_subcompetency', array('id' => $question->subcompetency, 'sessionid' => $session->id), '*');
$keywords = $DB->get_record('pr_question_keywords', array('question_id' => $qcm_id), 'keyword_id');
$reponses = $DB->get_records('pr_answer', array('question_id' => $qcm_id));
$globalcomment = $DB->get_records('pr_question', array('id' => $qcm_id, 'sessionid' => $session->id));

$keywords_list = [];
foreach ($keywords as $keyword_id) {
    $keyword = $DB->get_record('pr_keyword', array('id' => $keyword_id, 'sessionid' => $session->id), 'word');
    if ($keyword) {
...
$PAGE->set_url('/mod/peerreview/eval_qcm_view.php', array('id' => $id, 'prod_id' => $prod_id, 'qcm_id' => $qcm_id));
...
$evaluation = $DB->get_record('pr_evaluation', array(
    'question_id' => $qcm_id,
    'userid' => $userid
));
...
function generate_media_html($context, $filearea, $itemid, $file_storage) {
    $file_records = $file_storage->get_area_files($context->id, 'mod_peerreview', $filearea, $itemid, 'sortorder', false);
    $media_html = '';

    foreach ($file_records as $file) {
        if ($file->get_filename() == '.') continue;

        $file_url = moodle_url::make_pluginfile_url($context->id, 'mod_peerreview', $filearea, $itemid, $file->get_filepath(), $file->get_filename())->out();
...
echo "<div class='mx-auto'>";
    echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('production1', 'mod_peerreview') . "</p>";
echo "</div>";
...
echo "<div class='mx-auto mt-8'>";
    echo "<div class='mt-2 text-gray-600 text-sm flex flex-col space-y-1 rounded-3xl shadow p-4'>";
        ...
        echo "<span> <strong>" . get_string('referentiel', 'mod_peerreview') . ": </strong>" . ucfirst(($referentiel->name ?? 'N/A')) . "</span>";
        echo "</p>";

        // Compétence
        echo "<p class='flex items-center gap-2 text-lg'>";
        echo "<i class='fas fa-bookmark text-orange-500'></i>";
        echo "<span> <strong>" . get_string('competency', 'mod_peerreview') . ": </strong>" . ucfirst(($competency->name ?? 'N/A')) . "</span>";
        echo "</p>";

        // Sous-compétence
        echo "<p class='flex items-center gap-2 text-lg'>";
        echo "<i class='fas fa-award text-purple-500'></i>";
        echo "<span> <strong>" . get_string('subcompetency', 'mod_peerreview') . ": </strong>" . ucfirst(($subcompetency->name ?? 'N/A')) . "</span>";
        echo "</p>";

        // Mots clé
        echo "<p class='flex items-center gap-2 text-lg'>";
        echo "<i class='fas fa-thumbtack text-sky-500'></i>";
        echo "<span> <strong>" . get_string('subcompetency', 'mod_peerreview') . ": </strong>" . ucfirst($keywords_str) . "</span>";
        echo "</p>";
    echo "</div>";

    echo "<div class='rounded-3xl bg-indigo-200 my-4 p-4'>";
        echo "<label for='context_1' class='block font-bold text-gray-600 text-lg'>" . get_string('context', 'mod_peerreview') . " :</label>";
        ...
        if (!empty($img_text)) {
            ...
            echo "<p class='font-semibold text-md text-gray-600 inline-flex'>" . get_string('media_context', 'mod_peerreview') . " :</p>";
    ...
    echo "<div class='rounded-3xl bg-sky-200 my-4 p-4'>";
        echo "<label for='context_1' class='block font-bold text-gray-600 text-lg'>" . get_string('question', 'mod_peerreview') . " :</label>";
        ...
        echo "<i class='fas fa-magnifying-glass mr-2'></i> " . get_string('show_answer', 'mod_peerreview');
        echo "</button>";
        echo "</div>";
    ...
    echo "<div class='rounded-3xl bg-indigo-200 my-4 p-4'>";
        echo "<label for='context_1' class='block font-bold text-gray-600 text-lg'>" . get_string('global_comment', 'mod_peerreview') . " :</label>";
    ...
    echo "<div class='rounded-3xl bg-lime-200 my-4 p-4'>";
        echo "<label for='context_1' class='block font-bold text-gray-600 text-lg'>" . get_string('evaluate_comment', 'mod_peerreview') . " :</label>";
    ...
    echo "<div class='mt-4 text-right'>";
        echo '<button onclick="document.getElementById(\'evaluation-form\').submit();" class="inline-block px-4 py-2 font-semibold rounded-2xl bg-lime-200 hover:bg-lime-300 cursor-pointer text-lime-700 no-underline text-lg">' .get_string('submit_evaluation', 'mod_peerreview') . '</button>';
    echo "</div>";
