...
$PAGE->set_url('/mod/peerreview/student_dashboard_phase2.php', array('id' => $id));
...
$columns = [
    ...
    'completed_reviews' => get_string('completed_reviews', 'mod_peerreview'),
    'last_connected' => get_string('last_connected', 'mod_peerreview'),
    'correctors' => get_string('correctors', 'mod_peerreview'),
...
foreach ($students as $student) {
    $completed_reviews_count = $DB->count_records('pr_evaluation', array('userid' => $student->userid, 'status' => 1));
    $students_to_review = $DB->get_records('pr_assignedqcm', array('user_id' => $student->userid, 'sessionid' => $session->id));
    ...
    $colorClass = ($completed_reviews_count == 0) ? 'text-red-400' :
                  (($completed_reviews_count == $nbTotal_question) ? 'text-lime-500' : 'text-gray-600');
