...
$PAGE->set_url('/mod/peerreview/admin_add_user.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('user_gestion', 'mod_peerreview') . "</p>";
echo "</div>";
...
echo "<div class='flex mt-8 text-lg justify-between gap-4'>";
    ...
    echo '<i class="fas fa-user-plus mr-2"></i>' . get_string('student_gestion', 'mod_peerreview');
    echo '</a>';
    ...
    echo '<i class="fas fa-user-plus mr-2"></i>' . get_string('teacher_gestion', 'mod_peerreview');
    echo '</a>';
echo "</div>";
...
if ($gestion_type === 'student') {
    ...
    echo '<div class="mt-8 p-4 bg-indigo-50 rounded-3xl">';
        echo '<p class="font-bold text-center text-2xl text-indigo-400">' . get_string('add_student', 'mod_peerreview') . '</p>';

        echo "<form method='post' action='add_student.php?id={$id}' class='space-y-5'>";
            echo '<input type="hidden" name="id" value="' . $id . '">';

            echo '<div class="flex gap-4">';
                echo '<input type="text" name="firstname" required placeholder="' . get_string('firstname', 'mod_peerreview') . '" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400">';
                echo '<input type="text" name="lastname" required placeholder="' . get_string('lastname', 'mod_peerreview') . '" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400">';
            echo '</div>';

            echo '<div class="flex gap-4 w-full">';
                echo '<div class="w-full">';
                echo '<input type="email" name="email" required placeholder="' . get_string('email', 'mod_peerreview') . '" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400">';
                echo '</div>';

                echo '<div class="w-full py-2 ml-2">';
                echo '<div class="w-full flex items-center gap-2">';
                    echo '<label class="font-semibold text-gray-400 text-lg">' . get_string('tier_temps', 'mod_peerreview') . ' ?</label>';
        ...
        echo '<i class="fas fa-user-plus mr-2"></i>' . get_string('add', 'mod_peerreview');
        echo '</button>';
        echo '</div>';
    ...
    $columns = [
        'student_id' => get_string('student_id', 'mod_peerreview'),
        'full_name' => get_string('full_name', 'mod_peerreview'),
        'tier_temps ' => get_string('tier_temps', 'mod_peerreview'),
        'last_connected' => get_string('last_connected', 'mod_peerreview'),
    ...
    foreach ($students as $student) {
        ...
        echo '<td class="px-3 py-4 text-md text-gray-600">' . 
             ($student_name->lastaccess > 0 
                ...
                : mb_strtoupper(get_string('never_connected', 'mod_peerreview'), 'UTF-8')) . 
             '</td>';
        ...
        echo '<i class="fas fa-people-pulling mr-2"></i>' . mb_strtoupper(get_string('connect', 'mod_peerreview'), 'UTF-8');
        echo "</a>";
        echo '</td>';
    ...
    $teachers = $DB->get_records('pr_teachers', ['sessionid' => $session->id]);
    
    echo '<div class="mt-8 p-4 bg-indigo-50 rounded-3xl">';
        echo '<p class="font-bold text-center text-2xl text-indigo-400">' . get_string('add_teacher', 'mod_peerreview') . '</p>';
    ...
    $columns = [
        'teacher_id' => get_string('teacher_id', 'mod_peerreview'),
        'full_name' => get_string('full_name', 'mod_peerreview'),
        'last_connected' => get_string('last_connected', 'mod_peerreview'),
    ];
