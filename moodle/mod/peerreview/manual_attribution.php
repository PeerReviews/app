...
$PAGE->set_url('/mod/peerreview/manual_attribution.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('attribution_list', 'mod_peerreview') . "</p>";
echo "</div>";
...
echo "<div class='flex mt-8 text-lg justify-between gap-4'>";
    ...
    echo '<i class="fa-solid fa-user-graduate mr-2"></i>   ' . get_string('student_gestion', 'mod_peerreview');
    echo '</a>';
    ...
    echo '<i class="fa-solid fa-user-tie mr-2"></i>   ' . get_string('teacher_gestion', 'mod_peerreview');
    echo '</a>';
echo "</div>";
...
if ($gestion_type === 'student') {
    ...
    echo '<i class="fa-solid fa-sync mr-2"></i>' . get_string('trigger_attribution', 'mod_peerreview') . '</button>';
    ...
    echo '<i class="fa-solid fa-plus mr-2"></i>' . get_string('add_attribution', 'mod_peerreview') . '</button>';
    
    echo "</div>";

    // Récupération des étudiants
    $students = $DB->get_records('pr_assignedqcm', ['sessionid' => $session->id]);
    ...
    $columns = [
        ...
        'prod1' => get_string('prod1', 'mod_peerreview'),
        'prod2' => get_string('prod2', 'mod_peerreview'),
        'prod3' => get_string('prod3', 'mod_peerreview'),
    ...
    foreach ($students as $student) {
        ...
        echo '<td class="p-4 text-md text-gray-600">
                <button type="button" onclick="editRow(this, ' . $student->user_id . ')" class="px-4 py-2 min-w-40 bg-indigo-400 hover:bg-indigo-500 text-white text-md font-semibold rounded-2xl">' . get_string('edit', 'mod_peerreview') . '</button>
            </td>';
    ...
    echo "<div class='mb-4 mt-4 flex justify-end space-x-2 text-lg'>";
        ...
        . get_string('save_changes', 'mod_peerreview') . '</button>';
    echo "</div>";
    ...
    echo "<div class='flex mt-8 text-lg justify-center gap-4'>";
        ...
        echo '<button onclick="triggerTeacherAttribution()" class="block px-4 py-2 font-semibold rounded-2xl bg-amber-400 hover:bg-amber-500 text-white no-underline">'
            . get_string('trigger_attribution', 'mod_peerreview') . '</button>';
        ...
        echo '<button onclick="addTeacherRow()" class="inline-block px-4 py-2 font-semibold rounded-2xl bg-amber-400 hover:bg-amber-500 text-white no-underline">' 
            . get_string('add_attribution', 'mod_peerreview') . '</button>';
    echo "</div>";
    ...
    foreach ($teachers as $teacher) {
        ...
        // Ajout des boutons Modifier/Supprimer
        echo '<td class="p-4 text-md text-gray-600">
                <button type="button" onclick="editTeacherRow(this, ' . $teacher->id .', ' . $teacher->teacherid . ')" class="px-4 py-2 min-w-40 bg-indigo-400 hover:bg-indigo-500 text-white text-md font-semibold rounded-2xl">' . get_string('edit', 'mod_peerreview') . '</button>
            </td>';
...
function triggerStudentAttribution() {
    ...
    fetch('/mod/peerreview/trigger_student_attribution.php?sesskey=' + M.cfg.sesskey, {
        method: 'POST',
        headers: {
...
function triggerTeacherAttribution() {
    ...
    fetch('/mod/peerreview/trigger_teacher_attribution.php?sesskey=' + M.cfg.sesskey, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json'
