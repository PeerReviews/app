...
$PAGE->set_url('/mod/peerreview/admin_sessions.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('session_list', 'mod_peerreview') . "</p>";
echo "</div>";
...
echo "<p class='text-sm text-gray-600'>" . get_string('information_session', 'mod_peerreview') . "</p>"; 
echo "</div>";
echo "</div>";

// Récupération des sessions
$sessions = $DB->get_records('peerreview');
...
$columns = [
    'session_name' => get_string('session_name', 'mod_peerreview'),
    'time_created' => get_string('time_created', 'mod_peerreview'),
    'session_start' => get_string('session_start', 'mod_peerreview'),
    'session_end' => get_string('session_end', 'mod_peerreview'),
    'archived' => get_string('archived', 'mod_peerreview'),
    'actions' => get_string('actions', 'mod_peerreview')
];
...
foreach ($sessions as $session) {
    ...
    echo '<td class="px-3 py-4 text-md text-gray-600">' . 
        ($session->start_date_session > 0 
            ...
            : mb_strtoupper(get_string('NA', 'mod_peerreview'), 'UTF-8')) . 
        '</td>';
    echo '<td class="px-3 py-4 text-md text-gray-600">' . 
        ($session->end_date_session > 0 
            ...
            : mb_strtoupper(get_string('NA', 'mod_peerreview'), 'UTF-8')) . 
        '</td>';
    echo '<td class="px-3 py-4 text-md text-gray-600">' . ($session->archived == 1 ? get_string('yes', 'mod_peerreview') : get_string('no', 'mod_peerreview')) . '</td>';

    echo '<td class="p-4 text-md text-gray-600 flex items-center space-x-2">';
        echo '<a href="edit_session.php?id=' . $id . '&session_id=' . $session->id . '" class="px-4 py-2 bg-indigo-400 hover:bg-indigo-500 text-white text-md font-semibold rounded-2xl text-center" title="' . get_string('edit_session', 'mod_peerreview') . '">';
        ...
        echo '<form action="duplicate_session.php?id=' . $id . '" method="post" style="display:inline;">';
            ...
            echo '<button type="submit" class="px-4 py-2 bg-sky-400 hover:bg-sky-500 text-white text-md font-semibold rounded-2xl" title="' . get_string('duplicate_session', 'mod_peerreview') . '">';
        ...
        if ($session->archived == 1){
            ...
            echo '<form action="set_current_session.php?id=' . $id . '&session_id=' . $session->id . '" method="post" style="display:inline;">';
                echo '<input type="hidden" name="session_id" value="' . $session->id . '">';
                echo '<button type="submit" onclick="showActivateModal(' . $session->id . '); event.preventDefault();" class="px-4 py-2 bg-lime-400 hover:bg-lime-500 text-white text-md font-semibold rounded-2xl" title="' . get_string('activate_session', 'mod_peerreview') . '">';
            ...
            echo '<form action="archive_session.php?id=' . $id . '" method="post" style="display:inline;">';
                echo '<input type="hidden" name="session_id" value="' . $session->id . '">';
                echo '<button type="submit" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 text-white text-md font-semibold rounded-2xl" title="' . get_string('archive_session', 'mod_peerreview') . '">';
        ...
        echo '<form action="export_session.php?id=' . $id . '" method="post" style="display:inline;">';
                echo '<input type="hidden" name="session_id" value="' . $session->id . '">';
                echo '<button type="submit" class="px-4 py-2 bg-amber-400 hover:bg-amber-500 text-white text-md font-semibold rounded-2xl" title="' . get_string('export_session', 'mod_peerreview') . '">';
        ...
        echo '<form action="delete_session.php?id=' . $id . '&session_id=' . $session->id .'" method="post" style="display:inline;">';
            echo '<input type="hidden" name="session_id" value="' . $session->id . '">';
            echo '<button type="submit" onclick="showDeleteModal(' . $session->id . '); event.preventDefault();" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-md font-semibold rounded-2xl" title="' . get_string('delete_session', 'mod_peerreview') . '">';
...
<div id="delete-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
    <div class="bg-white rounded-3xl py-4 px-16 max-w-lg w-full">
        ...
        <div id="delete-message" class="mt-4 text-gray-700">
            <p><?php echo get_string('confirm_delete_session', 'mod_peerreview'); ?></p>
        </div>
...
<div id="activate-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
    <div class="bg-white rounded-3xl py-4 px-16 max-w-lg w-full">
        <div class="flex justify-between items-center">
            <h3 class="text-2xl font-semibold text-lime-400"><?php echo get_string('message_activate', 'mod_peerreview'); ?></h3>
        ...
        <div id="activate-message" class="mt-4 text-gray-700">
            <p><?php echo get_string('confirm_activate_session', 'mod_peerreview'); ?></p>
        </div>
        <div class="mt-2 text-right">
            <button id="confirm-activate-btn" class="px-4 py-2 bg-lime-400 text-white rounded-full hover:bg-lime-500 ml-2"><?php echo get_string('activate', 'mod_peerreview'); ?></button>
        </div>
    </div>
</div>
