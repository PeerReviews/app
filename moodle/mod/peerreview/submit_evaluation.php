...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($explanation) && $question_id > 0) {
        ...
        $existing_evaluation = $DB->get_record('pr_evaluation', array(
        ...
        if ($existing_evaluation) {
            ...
            $DB->update_record('pr_evaluation', $existing_evaluation);
        } else {
            ...
            $DB->insert_record('pr_evaluation', $record);
        }

        // Redirection avec message de succès
        redirect(new moodle_url('/mod/peerreview/eval_qcm_list.php', array('id' => $id, 'prod_id' => $prod_id)));
    } else {
        // Erreur : champs vides ou ID invalide
        redirect(new moodle_url('/mod/peerreview/eval_qcm_view.php', array('id' => $id, 'prod_id' => $prod_id, 'qcm_id' => $question_id)), 
