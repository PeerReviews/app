...
$evaluation = $DB->get_record('pr_evaluation', array('id' => $eval_id));
if ($evaluation) {
    $evaluation->grade = $grade;
    $DB->update_record('pr_evaluation', $evaluation);

    $questions = $DB->get_records('pr_question', array('userid' => $prod_id));
    ...
    foreach ($questions as $question) {
        $eval = $DB->get_record('pr_evaluation', array('question_id' => $question->id, 'userid' => $reviewer));
        if ($eval) {
            $evaluations[] = $eval;
        }
    }
