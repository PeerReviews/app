...
if (!empty($prod_id)) {
    ...
    $qs = $DB->get_records('pr_question', array('userid' => $prod_id, 'sessionid' => $session->id, 'status' => 1));
...
$PAGE->set_url('/mod/peerreview/eval_qcm_list.php', array('id' => $id, 'prod_id' => $prod_id));
...
if ($questions) {
    ...
    foreach ($questions as $question) {
        ...
        $evaluation = $DB->get_record('pr_evaluation', array('question_id' => $question->id, 'userid' => $USER->id));

        if ($evaluation && $evaluation->status == 1) {
            // Si la question a été évalué
            $buttonText = get_string('evaluated', 'mod_peerreview');
            $buttonIcon = 'fas fa-check-circle';
            $buttonColor = 'bg-lime-400 hover:bg-lime-500';
            $buttonTextColor = 'text-white';
        } else {
            // Si la question n'a pas été évalué
            $buttonText = get_string('to_be_evaluated', 'mod_peerreview');
        ...
        if ($question->poptypeid !== $previousPopTypeId) {
            $popInfo = $DB->get_record('pr_question_pop', array('id' => $question->poptypeid, 'sessionid' => $session->id));
    ...
    echo "<p class='text-center text-lg text-gray-600'>" . get_string('not_found', 'mod_peerreview') . "</p>";
}
