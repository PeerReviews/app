    ...
    foreach ($questions as $q_id => $question) {
        ...
        if (!empty(trim($question['question']))) {
            ...
            if ($pop_type_id !== 0){
                ...
                $pop_id = $DB->insert_record('pr_pop', $pop_record);
                if (!$pop_id) {
                    throw new moodle_exception('insertfailed', 'pr_pop');
                }
            ...
            $question_id = $DB->insert_record('pr_question', $question_record);

            if (!$question_id) {
                throw new moodle_exception('insertfailed', 'pr_question');
            }
            ...
            if (!empty($files[$area])) {
                ...
                foreach ($mdl_files as $mdl_file) {
                    ...
                    $real_url = moodle_url::make_pluginfile_url($context->id, 'mod_peerreview', 'contextfiles', $mdl_file->itemid, '/', $filename)->out();
            ...
            $updated = $DB->update_record('pr_question', $question_record);
            if (!$updated) {
            ...
            if (!empty($question['answers'])) {
                foreach ($question['answers'] as $answer) {
                    ...
                    try {
                        $inserted_answer_id = $DB->insert_record('pr_answer', $answer_record);
                        ...
                        if (!empty($files[$area])) {
                            ...
                            foreach ($mdl_files as $mdl_file) {
                                ...
                                if (intval($mdl_file->itemid) === $indexation) {
                                    ...
                                    $filename = $mdl_file->filename;
                                    $real_url = moodle_url::make_pluginfile_url($context->id, 'mod_peerreview', $area, $mdl_file->itemid, '/', $filename)->out();
                    ...
                    $updated = $DB->update_record('pr_answer', $answer_record);
                    if (!$updated) {
            ...
            if (!empty($question['keywords']) && is_array($question['keywords'])) {
                foreach ($question['keywords'] as $keyword_id) {
                    if (!empty(trim($keyword_id))) {
                        ...
                        $existing_relation = $DB->get_record('pr_question_keywords', array(
...
if (isset($_POST['submit'])) {
    redirect(new moodle_url('/mod/peerreview/qcm_list.php', array('id' => $id)), get_string('qcm_saved', 'mod_peerreview'), 2);
}

// Si la requête n'est pas un POST, rediriger
redirect(new moodle_url('/mod/peerreview/qcm_list.php', array('id' => $id)));
