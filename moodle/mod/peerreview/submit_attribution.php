...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['save_students']) && $_POST['save_students'] != ""){
        foreach ($_POST['save_students'] as $student_id => $studentData) {
            ...
            $attribution_id = $DB->insert_record('pr_assignedqcm', $record);
            if (!$attribution_id) {
                // En cas d'erreur
    ...
    if (isset($_POST['updatedData']) && $_POST['updatedData'] != "") {
        foreach ($_POST['updatedData'] as $updated_id => $updated) {
            ...
            $existingRecord = $DB->get_record('pr_assignedqcm', array('user_id' => $studentId, 'sessionid' => $session->id));
            ...
            if ($existingRecord) {
                ...
                $updated = $DB->update_record('pr_assignedqcm', $existingRecord);
    
                if (!$updated) {
                    print_r("Echec de l'update");
                    // En cas d'échec de la mise à jour
                    redirect(new moodle_url('/mod/peerreview/view.php', array('id' => $id)), '', 0);
                }
            } 
        }
    }

    redirect(new moodle_url('/mod/peerreview/manual_attribution.php', array('id' => $id)), '', 0);
}
else {
