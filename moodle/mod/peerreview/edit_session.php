...
$PAGE->set_url('/mod/peerreview/edit_session.php', ['id' => $id, 'session_id' => $session_id]);
$PAGE->set_title(format_string($session->name));
$PAGE->set_heading(format_string($course->fullname));
$PAGE->requires->css(new moodle_url('/mod/peerreview/style.css', ['v' => time()]));

echo $OUTPUT->header();

echo "<div class='mx-auto'>";
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('edit_session', 'mod_peerreview') . " " . format_string($session->name) . "</p>";
echo "</div>";
...
echo "<div class='border-b p-2' style='margin-bottom: 1rem !important'>";
    echo "<div class='flex text-center text-gray-600 items-end'>";
        echo "<p class='text-3xl font-semibold'> " . get_string('info_section_general', 'mod_peerreview') . "</p>";
    echo "</div>";
echo "</div>";
...
$fields = [
    ...
    'nbqcm' => get_string('nb_qcm', 'mod_peerreview'),
    'nbqcu' => get_string('nb_qcu', 'mod_peerreview'),
    'nbtcs' => get_string('nb_tcs', 'mod_peerreview'),
    'nbpop' => get_string('nb_pop', 'mod_peerreview'),
];
...
echo '<div class="grid grid-cols-1 gap-6 mt-6">';
    ...
    foreach ($fields as $field => $label) {
        if (!in_array($field, ['nbqcm', 'nbqcu', 'nbtcs', 'nbpop'])) {
            ...
            if ($field == 'referentiel') {
                $referentiel_name = $DB->get_field('pr_referentiel', 'name', ['id' => $session->$field]);
    ...
    echo "<div class='border-b p-2' style='margin-top: 4rem !important; margin-bottom: 1rem !important'>";
        echo "<div class='flex text-center text-gray-600 items-end'>";
            echo "<p class='text-3xl font-semibold'> " . get_string('info_section_question', 'mod_peerreview') . "</p>";
        echo "</div>";
    echo "</div>";
...
echo "<div class='border-b p-2' style='margin-top: 4rem !important;'>";
    echo "<div class='flex text-center text-gray-600 items-end'>";
        echo "<p class='text-3xl font-semibold'> " . get_string('info_section_date', 'mod_peerreview') . "</p>";
    echo "</div>";
echo "</div>";
...
foreach ($special_date_fields as $index => $field) {
    echo '<div class="flex flex-col">';
    echo '<label class="font-semibold mb-2 text-gray-600 text-lg">' . get_string($field, 'mod_peerreview') . ' : </label>';
...
foreach ($date_fields as $index => $field_group) {
    ...
    foreach ($field_group as $field) {
        echo '<div class="flex flex-col">';
        echo '<label class="font-semibold mb-2 ' . $text_color_class . ' text-lg">' . get_string($field, 'mod_peerreview') . ' : </label>';
...
$competencies = $DB->get_records('pr_competency', ['referentiel' => $session->referentiel, 'sessionid' => $session->id]);
...
<form action="save_session.php?id=<?= $id ?>&session_id=<?= $session->id ?>" method="post" class="session-form">
    ...
    <div class="border-b p-2" style="margin-top: 4rem !important;">
        <div class="flex text-center text-gray-600 items-end">
            <p class="text-3xl font-semibold"><?php echo get_string('info_section_competency', 'mod_peerreview'); ?></p>
        </div>    
    </div>
    ...
    <div class="flex justify-end mt-4">
        <button type="submit" class="inline-block px-4 py-2 font-semibold rounded-2xl bg-lime-400 hover:bg-lime-500 cursor-pointer text-white text-lg"><?php echo get_string('save', 'mod_peerreview'); ?></button>
    </div>
</form>
...
function editCompetenceField() {
    ...
    if (!competencies || competencies.length === 0) {
        let noCompetenceMessage = `
            <div class="p-4 text-center text-gray-600 bg-gray-100 border border-gray-300 rounded-lg mt-4">
                <p class="text-xl font-semibold"><?php echo get_string('competency_not_found', 'mod_peerreview'); ?></p>
            </div>
        `;
            ...
            <div id="competence-container${index_competence}" class="mt-4">
                <div class="flex justify-between items-center border-b pb-1">
                    ...
                    <button type="button" class="px-4 py-2 bg-red-500 text-white text-md font-semibold rounded-xl hover:bg-red-600" onclick="deleteCompetence(${index_competence})">
                        <i class="fas fa-trash-alt mr-2"></i> <?php echo get_string('delete', 'mod_peerreview'); ?>
                    </button>
                </div>
    ...
    let addButtonHTML = `
        <div class="flex justify-end mt-4">
            <button type="button" class="px-4 py-2 bg-indigo-400 text-white text-md font-semibold rounded-xl hover:bg-indigo-500" onclick="addCompetenceField()"><i class="fas fa-plus mr-2"></i> <?php echo get_string('add_competency', 'mod_peerreview'); ?></button>
        </div>
    `;
...
function addCompetenceField() {
    ...
    let fieldHTML = `
        <div id="competence-container${index_competence}" class="p-4 rounded-2xl bg-white mt-4">
            <p class="text-2xl text-gray-600 font-bold capitalize"> <?php echo get_string('newcompetency', 'mod_peerreview'); ?></p>
            
            <div class="flex items-center space-x-4 mt-4">
                <input type="text" id="competence-name${index_competence}" placeholder="<?php echo get_string('name_competence', 'mod_peerreview'); ?>" class="p-2 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-indigo-200" required>
                ...
                <button type="button" class="px-4 py-2 bg-indigo-400 text-white text-md font-semibold rounded-xl hover:bg-indigo-500 whitespace-nowrap capitalize" onclick="addSubCompetenceField(${index_competence})">
                    <?php echo get_string('add_subcompetences', 'mod_peerreview'); ?>
                </button>
            </div>
            ...
            <div class="flex mt-4 justify-end space-x-2">
                <button type="button" class="px-4 py-2 bg-lime-400 text-white text-md font-semibold rounded-xl hover:bg-lime-500 whitespace-nowrap capitalize" onclick="saveCompetence(${index_competence})">
                    <?php echo get_string('save', 'mod_peerreview'); ?>
                </button>
                <button type="button" class="px-4 py-2 bg-gray-400 text-white text-md font-semibold rounded-xl hover:bg-gray-500 whitespace-nowrap capitalize" onclick="deleteAddCompetence(${index_competence})">
                    <?php echo get_string('cancel', 'mod_peerreview'); ?>
                </button>
            </div>
        </div>
    `;
    ...
    let fieldHTML = `
        <div id="subcompetence-container${index_competence}${index_sub}" class="ml-6">
            <div class="flex items-center space-x-4">
                <input type="text" id="subcompetence-name${index_competence}${index_sub}" placeholder="<?php echo get_string('name_subcompetence', 'mod_peerreview'); ?>" class="p-2 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-indigo-200" required>
                ...
                <button type="button" class="px-4 py-2 bg-indigo-400 text-white text-md font-semibold rounded-xl hover:bg-indigo-500 whitespace-nowrap" onclick="addKeywordField(${index_competence}, ${index_sub})">
                    <?php echo get_string('add_keyword', 'mod_peerreview'); ?>
                </button>
            </div>
...
function addKeywordField(index_competence, index_sub) {
    ...
    let fieldHTML = `
        <input type="text" id="keyword${index_competence}${index_sub}${index_keyword}" class="mt-2 p-2 border rounded-xl w-full focus:outline-none focus:ring-2 focus:ring-indigo-200" placeholder="<?php echo get_string('name_keyword', 'mod_peerreview'); ?>" required>
    `;
