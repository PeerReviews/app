...
if (!empty($prod_id)) {
    $questions = $DB->get_records('pr_question', array('userid' => $prod_id, 'sessionid' => $session->id, 'status' => 1));
...
$PAGE->set_url('/mod/peerreview/teacher_production_eval.php', array('id' => $id, 'prod_id' => $prod_id));
...
echo "<div class='flex justify-between items-center gap-4 mt-4 text-xl text-gray-700 font-semibold text-center'>";
    echo "<div class='flex-1 text-center rounded-3xl shadow-md p-4 bg-gray-50'>";
        echo "<p>" . get_string('student', 'mod_peerreview') . " " . $prod_id . "</p>";
        echo "<p class='text-gray-700'>" . get_string('nb_evaluated_question', 'mod_peerreview') . " : 
    ...
    foreach ($user_evaluations as $userid_eval => $counts) {
        ...
        echo "<div class='flex-1 text-center rounded-3xl shadow-md p-4 bg-gray-50'>";
            echo "<p>" . get_string('student', 'mod_peerreview') . " " . $user_info->id . "</p>";
            echo "<p class='text-gray-700'>" . get_string('nb_evaluated_revision', 'mod_peerreview') . " : 
...
$grid_eval_qcu = $session->grid_eval_qcu;
$grid_eval_qcm = $session->grid_eval_qcm;
$grid_eval_tcs = $session->grid_eval_tcs;

function get_bonus_malus($grid_eval_id) {
    global $DB;
    return $DB->get_record('pr_grid_eval', array('id' => $grid_eval_id), '*');
}
...
foreach ($evaluation_types as $eval_type) {
    $grid_eval_id = $session->$eval_type;
    ...
    if ($grid_eval_record) {
        ...
        echo "<p class='text-center $title_color text-xl font-semibold'>" . get_string($eval_type, 'mod_peerreview') . "</p>";
        ...
        echo "<p>" . get_string('no_data', 'mod_peerreview') . "</p>";
        echo "</div>";
    }
}
...
if ($qcms) {
    ...
    foreach ($qcms as $index => $qcm) {
        ...
        $reponses = $DB->get_records('pr_answer', array('question_id' => $qcm->id));
        $evaluations = $DB->get_records('pr_evaluation', array('question_id' => $qcm->id));
        ...
        if ($qcm->poptypeid !== $previousPopTypeId) {
            $popInfo = $DB->get_record('pr_question_pop', array('id' => $qcm->poptypeid, 'sessionid' => $session->id));
                ...
                echo "<div class='flex flex-col justify-between p-4 w-full'>";
                    ...
                    echo "<div class='grid grid-cols-2 gap-6'>";
                        ...
                        echo "<div class='w-full'>"; 
                            ...
                            echo get_string('context', 'mod_peerreview');
                            echo "</p>";
                            ...
                            echo get_string('global_comment', 'mod_peerreview');
                            echo "</p>";
                        ...
                        foreach ($reponses as $reponse) {
                            ...
                            if (!empty($img_text_answer)) {
                                echo "<div class='flex items-center gap-2 mt-2'>";
                                echo "<p class='font-semibold text-md $answerColor inline-flex'>" . get_string('media_answer', 'mod_peerreview') . " :</p>";
                            ...
                            if (!empty($img_text_explanation)) {
                                echo "<div class='flex items-center gap-2 mt-2'>";
                                echo "<p class='font-semibold text-md $answerColor inline-flex'>" . get_string('media_explanation', 'mod_peerreview') . " :</p>";
                    ...
                    echo "<div class='bg-gray-50 rounded-lg p-4 my-2 flex items-center gap-4'>";
                        ...
                        echo "<span>" . get_string('note_for_question', 'mod_peerreview') . " :</span>";
                        echo "</p>";
                    ...
                    if ($evaluations) {
                        foreach ($evaluations as $evaluation) {
                            echo "<div class='bg-gray-50 p-4 rounded-lg flex flex-col h-full'>";
                                echo "<p class='text-gray-700 text-center text-lg mb-2 font-semibold'>" . get_string('student_revision', 'mod_peerreview') . " " . $evaluation->userid ."</p>";
                                echo "<p class='text-gray-500 mb-2'>{$evaluation->explanation}</p>";

                                echo "<div class='flex items-center gap-4 mt-auto'>";
                                    echo "<p class='text-gray-700 font-medium'>" . get_string('note_for_revision', 'mod_peerreview') . " :</p>";
                                    echo "<div class='flex gap-2'>";
                                        for ($i = 0; $i <= 5; $i++) {
                        ...
                        echo "<p class='text-gray-500'>" . get_string('no_evaluation_for_this_question', 'mod_peerreview') . "</p>";
                    }
                    echo "</div>";
                echo "</div>";
            echo "</div>";
        ...
        if ($index === 0) {
            echo "<div class='my-6 mx-4 text-center flex flex-col md:flex-row gap-4 justify-between items-center'>";
                ...
                echo "<span class='text-white font-bold'>" . get_string('autocomplete_question', 'mod_peerreview') . "</span>";
                echo "</button>";
                ...
                echo "<span class='text-white font-bold'>" . get_string('autocomplete_review', 'mod_peerreview') . "</span>";
                echo "</button>";
    ...
    echo "<p class='text-center text-lg text-gray-600'>" . get_string('qcm_not_found', 'mod_peerreview') . "</p>";
}
...
<div id="auto-complete-question-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
    <div class="bg-white rounded-3xl py-8 px-12 max-w-lg w-full shadow-xl transform transition-all scale-95 hover:scale-100 relative flex flex-col items-center justify-center">
        <h3 class="text-2xl font-semibold text-lime-500 my-4 text-center">
            <?php echo get_string('autocomplete_question_success', 'mod_peerreview'); ?>
        </h3>
...
<div id="auto-complete-review-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
    <div class="bg-white rounded-3xl py-8 px-12 max-w-lg w-full shadow-xl transform transition-all scale-95 hover:scale-100 relative flex flex-col items-center justify-center">
        <h3 class="text-2xl font-semibold text-lime-500 my-4 text-center">
            <?php echo get_string('autocomplete_review_success', 'mod_peerreview'); ?>
        </h3>
