...
$PAGE->set_url('/mod/peerreview/teacher_dashboard.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('teacher_list', 'mod_peerreview') . "</p>";
echo "</div>";
...
$columns = [
    ...
    'completed_revision' => get_string('completed_revision', 'mod_peerreview'),
...
foreach ($teachers as $teacher) {
    ...
    foreach ($productions as $production) {
        $to_grade = $DB->get_records('pr_question', array('userid' => $production->userid, 'sessionid' => $session->id, 'status' => 1));

        // Je dois récupérer le nombre de révisions fait par chaque élève $production->userid 
        $sql = "SELECT q.*, e.*
                FROM {pr_question} q
                JOIN {pr_evaluation} e ON q.id = e.question_id
