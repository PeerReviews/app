...
$PAGE->set_url('/mod/peerreview/admin_grid_eval.php', array('id' => $id));
...
echo "<p class='font-bold text-center text-3xl text-gray-600'>" . get_string('grid_eval_title', 'mod_peerreview') . "</p>";
echo "</div>";
...
if ($session) {
    ...
    $grid_eval_qcu = $DB->get_record('pr_grid_eval', ['id' => $session->grid_eval_qcu]);
    $grid_eval_qcm = $DB->get_record('pr_grid_eval', ['id' => $session->grid_eval_qcm]);
    $grid_eval_tcs = $DB->get_record('pr_grid_eval', ['id' => $session->grid_eval_tcs]);
    ...
    foreach ($grilles as $grille) {
        ...
        echo "<p class='font-bold text-2xl text-$colorText text-center'>" . get_string($titleString, 'mod_peerreview') . "</p>";
...
echo "<div class='flex mt-4 justify-center gap-4'>";
    echo "<button type='button' id='export-grid' class='w-full p-4 bg-indigo-300 text-white font-semibold rounded-2xl shadow-md hover:bg-indigo-400 transition text-center text-lg' onclick='exportGrid()'>";
        echo "<i class='fas fa-file-export mr-2'></i>" . get_string('export_grid', 'mod_peerreview');
    echo "</button>";
    ...
    echo "<button type='button' id='import-grid' class='w-full p-4 bg-indigo-300 text-white font-semibold rounded-2xl shadow-md hover:bg-indigo-400 transition text-center text-lg' onclick='importGrid()'>";
        echo "<i class='fas fa-file-import mr-2'></i>" . get_string('import_grid', 'mod_peerreview');
    echo "</button>";
echo "</div>";
...
echo "<button type='submit' id='save-grid' class='inline-block px-4 py-2 font-semibold rounded-2xl bg-lime-200 hover:bg-lime-300 cursor-pointer text-lime-700 no-underline text-lg' onclick='saveGrid()'>
        " . get_string('save', 'mod_peerreview') . "
        </button>";
echo "</form>";
echo "</div>";
...
<script>
    ...
    function importGrid() {
        ...
        let html = `
            <p><?php echo "" . get_string('import_texte', 'mod_peerreview') . "" ?></p>
