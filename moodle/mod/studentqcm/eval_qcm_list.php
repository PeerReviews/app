...
if (!empty($prod_id)) {
    ...
    $popsByType = array_reduce($pops, function($result, $pop) {
        $result[$pop->poptypeid][] = $pop;
        return $result;
    }, []);    
...
if ($qcms || $pops) {
    ...
    if ($pops){
        ...
        foreach ($popsByType as $poptypeid => $pop) {

            $nbqcm = $DB->get_record('question_pop', array('id' => $poptypeid), 'nbqcm')->nbqcm;
            $nbqcu = $DB->get_record('question_pop', array('id' => $poptypeid), 'nbqcu')->nbqcu;
            ...
            echo "<div class='bg-gray-100 p-4 rounded-3xl shadow'>";
            echo "<p> POP : " . $popText . "</p>";
            echo "</div>";
