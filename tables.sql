...
CREATE TABLE `mdl_studentqcm_assignedqcm` (
  ...
  `qcm_id` int(10) NOT NULL,
  PRIMARY KEY (`userId`, `qcm_id`),
  ...
  CONSTRAINT `fk_assignedqcm_qcmid` FOREIGN KEY (`qcm_id`) REFERENCES `mdl_studentqcm_question` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_assignedqcm_userid` FOREIGN KEY (`userId`) REFERENCES `mdl_user` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;

/*!40101 SET character_set_client = @saved_cs_client */;

-- Gestion des verrous pour l'import de données
